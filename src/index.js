       ...
       .then(country => {
           if (country.length > 1 && country.length < 10) {
               ...
               const countryMarkup = country.map(({ name, flags }) => 
                    `<li class = "country-list-element"><img src = "${flags.png}" alt = "${name.official}" width = "30" > <p> ${name.official} </p> </li>`)
                    .join('');
            ...
            //    console.log(langList);
               const oneCountryMarkup = country.map(({name, capital, flags, languages, population}) => 
                   `<div class = "country-list-element"> <img src = "${flags.png}" alt = "${name.official}" width = "30" > <h2> ${name.official} </h2> </div>
